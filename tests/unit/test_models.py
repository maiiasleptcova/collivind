...
def test_memory_node_creation():
    node = MemoryNode(content="Test content", summary="Test", category=MemoryCategory.FACT)
...
def test_entity_node_creation():
    node = EntityNode(name="PostgreSQL", type=EntityType.SERVICE)
...
def test_relationship_edge_creation():
    edge = RelationshipEdge(source_id="mem-1", target_id="ent-1", type=RelType.ABOUT)
