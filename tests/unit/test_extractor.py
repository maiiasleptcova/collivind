...
def test_parse_response_with_markdown_fence():
    response = "```json\n" + json.dumps([
        {"content": "test fact", "summary": "test", "category": "fact"}
    ]) + "\n```"
    results = parse_extraction_response(response)
    assert len(results) == 1
...
def test_parse_response_fixes_invalid_category():
    response = json.dumps([
        {"content": "test", "summary": "s", "category": "invalid_cat"}
    ])
    results = parse_extraction_response(response)
...
def test_parse_response_fixes_invalid_entity_type():
    response = json.dumps([
        {
            "content": "test", "summary": "s", "category": "fact",
...
def test_parse_response_clamps_confidence():
    response = json.dumps([
        {"content": "test", "summary": "s", "category": "fact", "confidence": 5.0}
    ])
    results = parse_extraction_response(response)
...
def test_parse_single_object_not_array():
    response = json.dumps(
        {"content": "single", "summary": "s", "category": "fact"}
    )
    results = parse_extraction_response(response)
    assert len(results) == 1
