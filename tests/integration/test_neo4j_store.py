...
def test_neo4j_lifecycle(store):
    ...
    # Create entity
    ent_create = EntityCreate(name="Neo4j", type=EntityType.SERVICE, properties={"language": "Java"})
    entity = store.create_entity(ent_create)
    assert entity.id == "neo4j"

    # Create relationship
    rel_create = RelationshipCreate(source_id=memory.id, target_id=entity.id, type=RelType.ABOUT, confidence=0.9)
